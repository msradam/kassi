...
- **Its autonomy is governable.** A Burr state machine refuses illegal steps and hash-chains
  every move. Safe to leave running unattended on ops infrastructure.
...
**One agent, two MCP servers, one audited state machine.** The workflow is a
[Burr](https://github.com/apache/burr) state machine served over MCP by
[Theodosia](https://msradam.github.io/theodosia/). It spawns the Grafana k6 MCP server and the
official Splunk MCP Server as hidden upstreams. The driving agent never sees them. It sees one
tool, `step(action, inputs)`, and the graph's edges are the only legal moves. An illegal step is
refused with the valid next actions, and every step and every refusal is written to an immutable,
hash-chained ledger. `kassi verify` proves the trail was not tampered with.
...
**Track.** Observability (primary). It also touches Platform & Developer Experience, since it
slots into the SDLC at the pull request.

**Splunk AI used at runtime.** The Splunk MCP Server (primary, entered for Best Use of Splunk MCP
Server) and the Splunk AI Toolkit (`StateSpaceForecast` + `anomalydetection`), both live against
Splunk Enterprise 10.4.0. To be precise about what we do *not* claim: kassi does not use Splunk
Hosted Models or the AI Assistant for SPL (it composes SPL in pure Python by design). Those are
listed under "What's next."

**Bonus prizes.** Best Use of Splunk MCP Server (the whole correlation runs through it); and Best
Use of Splunk Developer Tools (it builds on the MCP Server and AI Toolkit apps).

**New or significantly updated.** Significantly updated. kassi extends an earlier project,
Kassandra, which generated k6 load tests from a git diff. That diff-to-k6 idea is the shared part.
The rest is new: the audited Burr state machine over MCP, the official k6 2.0 MCP server, the
entire Splunk side (correlation, AI Toolkit forecasting, the independent groundedness audit, the
remediation diff, and publishing the run back to Splunk).

**How to test it.** The repo is public and open source (Apache-2.0).

- Offline, no setup: `uv sync && uv run pytest` runs the full state-machine suite against fakes
  for both MCP servers, with no k6, Splunk, or network.
- Full live run: [docs/SPLUNK_SETUP.md](docs/SPLUNK_SETUP.md) walks through a local Splunk and the
  Splunk MCP Server, then `uv run python scripts/verify_petclinic.py` drives the entire workflow
